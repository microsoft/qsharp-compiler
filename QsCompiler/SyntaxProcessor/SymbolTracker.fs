...
type SymbolTracker<'P>(globals : NamespaceManager, sourceFile, parent : QsQualifiedName, autoGeneratedFunctors : IEnumerable<QsFunctor>) = 
// TODO: once we support type specialiations, the parent needs to be the specialization name rather than the callable name
    ...
    let requiredFunctorSupport = autoGeneratedFunctors.ToImmutableHashSet()
    ...
    /// contains a "stack" of all local declarations up to this point in each scope
    let mutable localVariables : Dictionary<NonNullable<string>, LocalVariableDeclaration<NonNullable<string>>> list = []
    ...
    let localVariableWithName (name : NonNullable<string>) = 
        let rec varDecl = function 
            | [] -> Null
            | (head : Dictionary<_,_>) :: _ when head.ContainsKey name -> Value head
            | _ :: tail -> varDecl tail
        varDecl localVariables
    ...
    /// contains the set of functors that need to be supported by each operation call within (any of) the scope(s) of this symbol tracker
    member internal this.RequiredFunctorSupport = requiredFunctorSupport
    /// returns true if no scope is currently open
    member this.AllScopesClosed = localVariables.Length = 0

    /// pushes a new scope onto the stack and opens it
    member this.BeginScope() = 
        localVariables <- new Dictionary<_,_>() :: localVariables

    /// pops the most recent scope from the stack, thus closing it
    member this.EndScope() = 
        if localVariables.Length = 0 then InvalidOperationException "no scope is currently open" |> raise
        localVariables <- localVariables.Tail
    ...
    member this.TryAddVariableDeclartion (decl : LocalVariableDeclaration<NonNullable<string>>) =
        ...
        else localVariables.Head.Add(decl.VariableName, decl); true, [||]
    ...
    member this.CurrentDeclarations = 
        let varDeclarations = seq { for scope in localVariables do for decl in scope.Values do yield decl }
