...
namespace microsoft {
namespace quantum {
llvm::PreservedAnalyses ExpandStaticAllocationPass::run(llvm::Function &               function,
                                                        llvm::FunctionAnalysisManager &fam)
{
  ...
  for (auto &basic_block : function)
  {
    ...
    for (auto &instruction : basic_block)
    {
      ...
      {
        uint32_t idx = 0;
        uint32_t N   = static_cast<uint32_t>(callee_function->arg_size());
...
llvm::Function *ExpandStaticAllocationPass::ExpandFunctionCall(
    QubitAllocationResult const &depenency_graph, llvm::Function &callee,
