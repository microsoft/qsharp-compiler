...
def run_clang_tidy(build_dir: str, filename: str,
                   fix_issues: bool = False) -> bool:
    """
    ...
    output_file = os.path.abspath(
        os.path.join(build_dir, 'clang_tidy_fixes.yaml'))

    cmd.append('-header-filter=".*\\/(Passes)\\/(qir\\/qat)\\/.*"')
...
def main_cpp(fix_issues: bool) -> bool:
    ...
    source_dir = os.path.join(PROJECT_ROOT, "qir", "qat")
    generator = None
    extensions = ["cpp"]
    ...
    for filename in files_to_analyse:
        # Ignoring tests for now to keep CI times down
        if "/Tests/" in filename or "\\Tests\\" in filename:
            continue

        success = success and run_clang_tidy(
            build_dir, filename, fix_issues=fix_issues)
    return success
