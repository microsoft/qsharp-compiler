...
def require_token(token: str, filename: str, contents: str, cursor: int, fix_issues: bool) -> int:
    """
...
def require_pragma_once(filename: str, contents: str, cursor: int, fix_issues: bool) -> int:
    """
...
def enforce_cpp_license(filename: str, contents: str, cursor: int, fix_issues: bool) -> int:
    """
...
def enforce_py_license(filename: str, contents: str, cursor: int, fix_issues: bool) -> int:
    """
...
def enforce_formatting(filename: str, contents: str, cursor: int, fix_issues: bool) -> int:
    """
...
def enforce_quantum_symbols(filename: str, contents: str, cursor: int, fix_issues: bool) -> int:
    fail = False
    for match in QUANTUM_SYMBOL_PATTERN.finditer(contents):
        ...
        if type == "qis":
            if instr not in VALID_INSTRUCTIONS:
                logger.error("Invalid instruction '{}' in {}".format(instr, filename))
                logger.info("Experimental feature - style check will not fail yet")
                # fail = True
            elif specialisation not in ["body", "ctl", "adj", "adjctl"]:
                logger.error("Missing specialisation for quantum symbol: {} in {}".format(full, filename))
                logger.info("Experimental feature - style check will not fail yet")
                # fail = True

        if (start == '"' or end == '"') and start != end:
            logger.error("Invalid string format for: '{}{}{}' in {}".format(start, full, end, filename))
            logger.info("Experimental feature - style check will not fail yet")
...
SOURCE_PIPELINES = [
    ...
    {
        "name": "Examples",
        "src": path.join(PROJECT_ROOT, "examples"),
...
def execute_pipeline(pipeline: IPipeline, filename: str, fix_issues: bool) -> bool:
    """
