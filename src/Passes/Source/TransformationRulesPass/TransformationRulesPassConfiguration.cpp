...
void TransformationRulesPassConfiguration::setup(ConfigurationManager &config)
{
  ...
  config.addParameter(clone_functions_, "clone-functions",
                      "Clone functions to ensure correct qubit allocation.");
  config.addParameter(transform_execution_path_only_, "transform-execution-path-only",
  ...
  config.addParameter(reuse_results_, "reuse-results",
  ...
  config.addParameter(simplify_prior_transformation_, "simplify-prior-transform",
                      "When active, the IR is simplified using LLVM passes before transformation.");

  // Not implemented yet
  config.addParameter(group_measurements_, "group-measurements",
                      "NOT IMPLEMENTED - group-measurements");
  config.addParameter(one_shot_measurement_, "one-shot-measurement",
...
bool TransformationRulesPassConfiguration::operator==(
    TransformationRulesPassConfiguration const &ref) const
{

  return (delete_dead_code_ == ref.delete_dead_code_ && clone_functions_ == ref.clone_functions_ &&
