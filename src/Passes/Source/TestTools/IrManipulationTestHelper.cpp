...
{
  module_ =
      llvm::parseIR(llvm::MemoryBufferRef(data, "IrManipulationTestHelper"), error_, context_);
...
void IrManipulationTestHelper::applyProfile(ProfilePtr const &       profile,
                                            OptimizationLevel const &optimisation_level, bool debug)
{
  auto module_pass_manager =
      profile->createGenerationModulePass(pass_builder_, optimisation_level, debug);
...
IrManipulationTestHelper::String IrManipulationTestHelper::generateScript(String const &body,
                                                                          String const &args) const
{
...
bool IrManipulationTestHelper::isModuleBroken()
{
  ...
  auto                   result = verifier.run(*module_, moduleAnalysisManager());
