...
template <typename T>
ConfigBind<T>::ConfigBind(Type &bind, T default_value, String const &name,
...
template <typename T>
template <typename R>
void ConfigBind<T>::alterNameBasedOnType(R const &default_value)
{
  std::stringstream ss{""};
  ss << default_value;
  setDefault(ss.str());
...
template <typename T>
void ConfigBind<T>::alterNameBasedOnType(bool const &default_value)
{
  markAsFlag();
...
template <typename T>
bool ConfigBind<T>::setupArguments(ParameterParser &parser)
{
...
template <typename T>
template <typename R>
bool ConfigBind<T>::setupArguments(ParameterParser &, R const &)
{
  return true;
}

template <typename T>
bool ConfigBind<T>::setupArguments(ParameterParser &parser, bool const &)
{
  parser.addFlag(name());
  return true;
}
...
template <typename T>
bool ConfigBind<T>::configure(ParameterParser const &parser)
{
...
template <typename T>
typename ConfigBind<T>::String ConfigBind<T>::value()
{
...
template <typename T>
template <typename A>
typename ConfigBind<T>::String ConfigBind<T>::valueAsString(A const &)
{
  std::stringstream ss{""};
  ss << bind_;
  return ss.str();
...
template <typename T>
template <typename A>
void ConfigBind<T>::loadValue(ParameterParser const &     parser,
                              EnableIf<A, bool, A> const &default_value)
{
  bind_ = default_value;
  if (parser.has(name()))
  {
...
template <typename A>
void ConfigBind<T>::loadValue(ParameterParser const &       parser,
                              EnableIf<A, String, A> const &default_value)
{
  bind_ = default_value;
