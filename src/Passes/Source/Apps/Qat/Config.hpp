...
namespace quantum
{
    struct QatConfig
    {
        ...
        void setup(ConfigurationManager& config)
        {
            ...
            config.addParameter(
                generate, "generate", "Transforms the IR in correspondance with the specified transformation.");
            ...
            config.addParameter(verify_module, "verify-module", "Verifies the module after transformation.");

            config.addParameter(dump_config, "dump-config", "Prints the configuration to the standard output.");
        }
