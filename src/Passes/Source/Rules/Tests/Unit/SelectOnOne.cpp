...
TEST(RuleSetTestSuite, SelectOnOne)
{
  ...
  EXPECT_TRUE(
      ir_manip->hasInstructionSequence({"%0 = tail call i1 @__quantum__qir__read_result(%Result* "
                                        "nonnull inttoptr (i64 1 to %Result*))",
                                        "%1 = select i1 %0, i8 5, i8 6"}));

  EXPECT_FALSE(ir_manip->hasInstructionSequence(
                   {"%1 = call i1 @__quantum__rt__result_equal(%Result* nonnull inttoptr (i64 1 "
                    "to %Result*), %Result* %0)"}) ||
               ir_manip->hasInstructionSequence(
                   {"%1 = tail call i1 @__quantum__rt__result_equal(%Result* nonnull inttoptr "
