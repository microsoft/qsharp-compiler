...
{
  if (depth_ >= config_.max_recursion)
  {
    llvm::errs() << "Exceed max recursion of " << config_.max_recursion << "\n";
    return false;
  }
  ++depth_;
...
void ProfilePass::runCopyAndExpand(llvm::Module &module, llvm::ModuleAnalysisManager &)
{
  ...
  {
    if (function.hasFnAttribute(config_.entry_point_attr))
    {
...
llvm::PreservedAnalyses ProfilePass::run(llvm::Module &module, llvm::ModuleAnalysisManager &mam)
{
  ...
  // Applying rule set
  if (config_.transform_execution_path_only)
  {
