...
type QsExpression with
    ...
    static member New (value, range) = { Expression = value; Range = Value range }
...
type QsSpecializationGenerator with
    ...
    static member New (value, range) = { Generator = value; TypeArguments = Null; Range = range }
...
type Characteristics with
    ...
    static member New (kind, range) = { Characteristics = kind; Range = Value range }

    static member New (kind, range) = { Characteristics = kind; Range = range }
