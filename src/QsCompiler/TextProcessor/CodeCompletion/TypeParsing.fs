...
let characteristicsAnnotation =
    let rec characteristicsExpression =
        parse {
            let infixOp =
                operator qsSetUnion.op ""
                <|> operator qsSetIntersection.op ""
    ...
    expectedKeyword qsCharacteristics
...
let rec nonArrayType =
    let typeParameter =
        pchar '\''
        >>. expectedId TypeParameter (term symbol)
    ...
    let operationType =
        ...
        brackets
            (lTuple, rTuple)
            (attempt
                (brackets (lTuple, rTuple) inOutType
    ...
    choice [ attempt typeParameter
             ...
             keywordType
             <|>@ expectedQualifiedSymbol UserDefinedType ]
...
and qsType =
    parse {
        return!
            nonArrayType
            .>> many (brackets (lArray, rArray) (emptySpace >>% []))
    }
