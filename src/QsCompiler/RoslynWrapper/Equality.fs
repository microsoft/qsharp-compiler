...
module EqualityOperators =
    ...
    let private setExpressionBody body (co: OperatorDeclarationSyntax) = co.WithExpressionBody body
    ...
    let ``operator ==`` (left, right, vtype) initializer =
        (SyntaxKind.BoolKeyword |> (SyntaxFactory.Token >> SyntaxFactory.PredefinedType),
         SyntaxKind.EqualsEqualsToken |> SyntaxFactory.Token)
        |> SyntaxFactory.OperatorDeclaration
        |> setParameterList [ (left, vtype)
                              (right, vtype) ]
    ...
    let ``operator !=`` (left, right, vtype) initializer =
        (SyntaxKind.BoolKeyword |> (SyntaxFactory.Token >> SyntaxFactory.PredefinedType),
         SyntaxKind.ExclamationEqualsToken |> SyntaxFactory.Token)
        |> SyntaxFactory.OperatorDeclaration
        |> setParameterList [ (left, vtype)
                              (right, vtype) ]
        |> setModifiers [ ``public``; ``static`` ]
