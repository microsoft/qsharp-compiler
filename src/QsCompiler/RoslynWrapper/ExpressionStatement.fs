...
module Expressions =
    ...
    // await expr
    let await = SyntaxFactory.AwaitExpression
    ...
    let ``() => {}`` parameters block =
        block |> Seq.toArray |> SyntaxFactory.Block |> ``() =>`` parameters

    let ``_ =>`` parameterName expression =
        SyntaxFactory.SimpleLambdaExpression(
            parameterName |> (SyntaxFactory.Identifier >> SyntaxFactory.Parameter),
            expression
        )

    // make a statement from an expression
    let statement s =
        SyntaxFactory.ExpressionStatement s :> Syntax.StatementSyntax

    // left ?? right
    let (<??>) left right =
        SyntaxFactory.BinaryExpression(SyntaxKind.CoalesceExpression, left, right) :> ExpressionSyntax

    // left.right
    let (<|.|>) left right =
        SyntaxFactory.MemberAccessExpression(SyntaxKind.SimpleMemberAccessExpression, left, right) :> ExpressionSyntax
    ...
    // -(expr)
    let ``-`` expr =
        (SyntaxKind.UnaryMinusExpression, SyntaxFactory.ParenthesizedExpression expr)
        |> SyntaxFactory.PrefixUnaryExpression
        :> ExpressionSyntax

    // left + right
    let (<+>) left right =
        (SyntaxKind.AddExpression, left, right) |> SyntaxFactory.BinaryExpression :> ExpressionSyntax

    // left - right
    let (<->) left right =
        (SyntaxKind.SubtractExpression, left, right) |> SyntaxFactory.BinaryExpression :> ExpressionSyntax

    // left * right
    let (<*>) left right =
        (SyntaxKind.MultiplyExpression, left, right) |> SyntaxFactory.BinaryExpression :> ExpressionSyntax

    // left / right
    let (</>) left right =
        (SyntaxKind.DivideExpression, left, right) |> SyntaxFactory.BinaryExpression :> ExpressionSyntax

    // left % right
    let (<%>) left right =
        (SyntaxKind.ModuloExpression, left, right) |> SyntaxFactory.BinaryExpression :> ExpressionSyntax

    // left ^ right
    let (<^>) left right =
        (SyntaxKind.ExclusiveOrExpression, left, right) |> SyntaxFactory.BinaryExpression :> ExpressionSyntax

    // left == right
    let (.==.) left right =
        (SyntaxKind.EqualsExpression, left, right) |> SyntaxFactory.BinaryExpression :> ExpressionSyntax

    // left != right
    let (.!=.) left right =
        (SyntaxKind.NotEqualsExpression, left, right) |> SyntaxFactory.BinaryExpression :> ExpressionSyntax

    // left <= right
    let (.<=.) left right =
        (SyntaxKind.LessThanOrEqualExpression, left, right) |> SyntaxFactory.BinaryExpression :> ExpressionSyntax

    // left < right
    let (.<.) left right =
        (SyntaxKind.LessThanExpression, left, right) |> SyntaxFactory.BinaryExpression :> ExpressionSyntax

    // left >= right
    let (.>=.) left right =
        (SyntaxKind.GreaterThanOrEqualExpression, left, right) |> SyntaxFactory.BinaryExpression :> ExpressionSyntax

    // left > right
    let (.>.) left right =
        (SyntaxKind.GreaterThanExpression, left, right) |> SyntaxFactory.BinaryExpression :> ExpressionSyntax

    // left && right
    let (.&&.) left right =
        (SyntaxKind.LogicalAndExpression, left, right) |> SyntaxFactory.BinaryExpression :> ExpressionSyntax

    // left &&& right
    let (.&&&.) left right =
        (SyntaxKind.BitwiseAndExpression, left, right) |> SyntaxFactory.BinaryExpression :> ExpressionSyntax

    // left || right
    let (.||.) left right =
        (SyntaxKind.LogicalOrExpression, left, right) |> SyntaxFactory.BinaryExpression :> ExpressionSyntax

    // left ||| right
    let (.|||.) left right =
        (SyntaxKind.BitwiseOrExpression, left, right) |> SyntaxFactory.BinaryExpression :> ExpressionSyntax
    ...
    // left >>> right
    let (.>>>.) left right =
        // TODO: Should be replaced with a use of SyntaxFactory.Token to generate an unsigned shift right. For reference,
        ...
        // Alternatively, if https://github.com/dotnet/roslyn/issues/61854 is resolved then we can use new Roslyn syntax directly.
        (SyntaxKind.RightShiftExpression, left, right) |> SyntaxFactory.BinaryExpression :> ExpressionSyntax

    // left <<< right
    let (.<<<.) left right =
        (SyntaxKind.LeftShiftExpression, left, right) |> SyntaxFactory.BinaryExpression :> ExpressionSyntax

    // ! (expr)
    let (!) expr =
        (SyntaxKind.LogicalNotExpression, SyntaxFactory.ParenthesizedExpression expr)
        |> SyntaxFactory.PrefixUnaryExpression
        :> ExpressionSyntax

    // bitwise NOT
    let ``~~~`` expr =
        (SyntaxKind.BitwiseNotExpression, SyntaxFactory.ParenthesizedExpression expr)
        |> SyntaxFactory.PrefixUnaryExpression
        :> ExpressionSyntax

    // expr is target
    let ``is`` targetType expression =
        SyntaxFactory.BinaryExpression(SyntaxKind.IsExpression, expression, ident targetType) :> ExpressionSyntax
    ...
    let ``((`` expr ``))`` =
        SyntaxFactory.ParenthesizedExpression expr :> ExpressionSyntax

    // single line comment
    let ``//`` comment node =
        [ ("//" + comment |> SyntaxFactory.Comment); (SyntaxFactory.EndOfLine "") ]
    ...
    let ``#lineNr`` (lineNumber: int) (file: string) =
        SyntaxFactory.Trivia(
            SyntaxFactory.LineDirectiveTrivia(SyntaxFactory.Literal(lineNumber), SyntaxFactory.Literal(file), true)
        )
...
module Statements =
    ...
    // throw s;
    let ``throw`` eOpt =
        eOpt |> Option.fold (fun _ e -> SyntaxFactory.ThrowStatement e) (SyntaxFactory.ThrowStatement())
        :> StatementSyntax

    // return;
    // return s;
    let ``return`` eOpt =
        eOpt |> Option.fold (fun _ e -> SyntaxFactory.ReturnStatement e) (SyntaxFactory.ReturnStatement())
        :> StatementSyntax
    ...
    let ``{{`` blocks ``}}`` =
        blocks |> (Seq.toArray >> SyntaxFactory.Block) :> Syntax.StatementSyntax
