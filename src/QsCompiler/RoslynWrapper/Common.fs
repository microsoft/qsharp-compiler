...
module Common =
    ...
    let ``ident`` s =
        (SyntaxFactory.Identifier >> SyntaxFactory.IdentifierName) s

    let ``:=`` expression =
        expression |> SyntaxFactory.EqualsValueClause

    let ``=>`` expression =
        expression |> SyntaxFactory.ArrowExpressionClause

    let ``param`` paramName ``of`` paramType =
        let p = paramName |> (SyntaxFactory.Identifier >> SyntaxFactory.Parameter)
