...
module CompilationUnit =
    ...
    let assembly = SyntaxFactory.Token(SyntaxKind.AssemblyKeyword) |> SyntaxFactory.AttributeTargetSpecifier
    ...
    let private addMembers members (cu: CompilationUnitSyntax) =
        members |> (Seq.toArray >> SyntaxFactory.List) |> cu.WithMembers

    let private addAttributes att (cu: CompilationUnitSyntax) =
        att |> (Seq.toArray >> SyntaxFactory.List) |> cu.WithAttributeLists
    ...
    let ``pragmaDisableWarning`` warning (cu: CompilationUnitSyntax) =
        ...
        cu.WithLeadingTrivia [| yield pr
                                yield SyntaxFactory.CarriageReturnLineFeed
                                for trivia in cu.GetLeadingTrivia() do
                                    yield trivia |]

    let ``with leading comments`` comments (cu: CompilationUnitSyntax) =
        cu.WithLeadingTrivia [| for comment in comments |> Seq.map SyntaxFactory.Comment do
                                    yield comment
                                    yield SyntaxFactory.CarriageReturnLineFeed
                                for trivia in cu.GetLeadingTrivia() do
                                    yield trivia |]
