...
type internal FunctionEvaluator(callables: IDictionary<QsQualifiedName, QsCallable>) =
    ...
    member private this.EvaluateExpression expr : Imp<TypedExpression> =
        imperative {
            let! vars, counter = getState
            let result = ExpressionEvaluator(callables, vars, counter / 2).OnTypedExpression expr
...
and internal ExpressionEvaluator (callables: IDictionary<QsQualifiedName, QsCallable>, constants: IDictionary<string, TypedExpression>, stmtsLeft: int) =
