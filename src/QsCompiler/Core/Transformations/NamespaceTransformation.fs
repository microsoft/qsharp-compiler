...
type NamespaceTransformationBase internal (options: TransformationOptions, _internal_) =
    ...
    member private this.OnCallableKind(c: QsCallable) =
        ...
        |> Node.BuildOr c (c.FullName, attributes, c.Visibility, argTuple, signature, specializations, doc, comments)
    ...
    default this.OnTypeDeclaration t =
        ...
        |> Node.BuildOr t (t.FullName, attributes, t.Visibility, typeItems, underlyingType, doc, comments)
