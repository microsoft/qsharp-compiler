...
module SyntaxGenerator =
    ...
    let AutoGeneratedExpression kind exTypeKind qDep =
        let inferredInfo =
            InferredExpressionInformation.New(false, quantumDep = qDep)
    ...
    let TupleLiteral (items: TypedExpression seq) =
        ...
        let tupleType =
            items
            |> Seq.map (fun item -> item.ResolvedType)
    ...
    let private CallNonGeneric (lhs: TypedExpression, rhs: TypedExpression) =
        ...
        | _ ->
            ArgumentException "given lhs is not callable"
            |> raise
    ...
    let private ReverseRange (ex: TypedExpression) =
        let buildCallToReverse ex =
            let kind =
                Identifier.GlobalCallable BuiltIn.RangeReverse.FullName
        ...
        | _ ->
            ArgumentException "given expression is not a range"
            |> raise
    ...
    let private Length (ex: TypedExpression) =
        ...
        let typeParameterName =
            ...
            | _ ->
                ArgumentException "Length is expected to be a function"
                |> raise

        let typeParameter =
            QsTypeParameter.New(callableName, typeParameterName, Null)

        let genArrayType =
            QsTypeKind.ArrayType
                (QsTypeKind.TypeParameter typeParameter
                 |> ResolvedType.New)
            |> ResolvedType.New

        let exTypeKind =
            QsTypeKind.Function(genArrayType, QsTypeKind.Int |> ResolvedType.New)
    ...
    let LengthMinusOne (ex: TypedExpression) =
        ...
        let kind =
            QsExpressionKind.SUB(callToLength, IntLiteral 1L)
    ...
    let ReverseIterable (ex: TypedExpression) =
        ...
        | _ ->
            ArgumentException "the given expression is not iterable"
            |> raise
    ...
    let IsNegative (ex: TypedExpression) =
        let kind =
            ...
            | _ ->
                ArgumentException "the type of the given expression does not support arithmetic operations"
                |> raise
    ...
    let WithoutRangeInfo (signature: ResolvedSignature) =
        let argType =
            signature.ArgumentType |> StripPositionInfo.Apply

        let returnType =
            signature.ReturnType |> StripPositionInfo.Apply
    ...
    let AddControlQubits (argT: ResolvedType) =
        [ QubitArray; argT ].ToImmutableArray()
        |> TupleType
        |> ResolvedType.New
    ...
    let WithControlQubits arg offset (name, symRange: QsNullable<_>) =
        ...
        let ctlQs =
            LocalVariableDeclaration.New false ((offset, range), name, QubitArray, false)

        let unitArg =
            let argName = ValidName InternalUse.UnitArgument
            let unitT = UnitType |> ResolvedType.New

            LocalVariableDeclaration.New false ((offset, range), argName, unitT, false)
            |> QsTupleItem // the range is not accurate here, but also irrelevant
        ...
        | _ ->
            ArgumentException "expecting the given argument tuple to be a QsTuple"
            |> raise
        |> QsTuple
    ...
    let ArgumentWithControlQubits (arg: TypedExpression) (ctlQs: TypedExpression) =
        ...
        if ctlQs.ResolvedType.Resolution
           <> QubitArray.Resolution
           && not (ctlQs.ResolvedType |> isInvalid) then
            new ArgumentException "expression for the control qubits is valid but not of type Qubit[]"
            |> raise
    ...
    let ImmutableQubitArrayWithName name =
        let kind =
            ...
            |> QsExpressionKind.Identifier

        let exTypeKind =
            QsTypeKind.ArrayType(QsTypeKind.Qubit |> ResolvedType.New)
    ...
    let ControlledOperation (ex: TypedExpression) =
        ...
        let kind =
            QsExpressionKind.ControlledApplication ex
        ...
        | QsTypeKind.Operation ((it, ot), opInfo) ->
            ...
            | _ ->
                ArgumentException "given expression does not correspond to a suitable operation"
                |> raise
        | _ ->
            ArgumentException "given expression does not correspond to a suitable operation"
            |> raise
