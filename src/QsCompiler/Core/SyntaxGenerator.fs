...
and public StripPositionInfo() as this =
    inherit SyntaxTreeTransformation()
    static let defaultInstance = StripPositionInfo()

    do
        this.Types <- StripPositionInfoFromType(this)
        this.Expressions <- StripPositionInfoFromExpression(this)
        this.Statements <- StripPositionInfoFromStatement(this)
        this.Namespaces <- StripPositionInfoFromNamespace(this)
...
module SyntaxGenerator =
    ...
    let internal fullyQualifiedName = Regex(@"^[\p{L}_][\p{L}\p{Nd}_]*(\.[\p{L}_][\p{L}\p{Nd}_]*)+$")
    ...
    /// Creates a typed expression that corresponds to a Pauli literal.
    /// Sets the range information for the built expression to Null.
    let PauliLiteral v =
        AutoGeneratedExpression (PauliLiteral v) QsTypeKind.Pauli false
    ...
    /// Creates a typed expression that represents a local variable with the give name and type kind.
    /// The given parameter qDep defines whether the variable (potentially) depends on quantum computations.
    let LocalVariable variableName varTypeKind qDep =
        let id = Identifier(LocalVariable variableName, Null)
        AutoGeneratedExpression id varTypeKind qDep

    /// Creates a typed expression that represents a globally declared callable with the give name and type kind.
    let GlobalCallable callableName callableType typeArguments =
        let id = Identifier(GlobalCallable callableName, typeArguments)
        AutoGeneratedExpression id callableType false
    ...
    let ArgumentTupleAsExpression (argTuple: QsTuple<LocalVariableDeclaration<QsLocalSymbol>>) =
        let rec resolveArgTupleItem =
            ...
            | QsTupleItem (decl: LocalVariableDeclaration<QsLocalSymbol>) ->
                ...
                |> function
                    | ValidName varName ->
                        let exKind = (Identifier.LocalVariable varName, Null) |> Identifier
    ...
    let CallNonGeneric (lhs: TypedExpression, rhs: TypedExpression) =
        ...
        let quantumDep = // NOTE: for an operation call we always need to set the quantum dependency to true
        ...
        | QsTypeKind.Operation ((_, ot), _) -> AutoGeneratedExpression kind ot.Resolution true
    ...
    let private reverseArray (ex: TypedExpression) =
        let built =
            let reversingRange =
                rangeExpression (rangeExpression (LengthMinusOne ex, IntLiteral -1L), IntLiteral 0L)
    ...
    /// Returns an expression that adds the two expressions.
    ...
    let ConditionalExpression exType (condition : TypedExpression, onTrue, onFalse) =
        ...
        let quantumDep =
            condition.InferredInformation.HasLocalQuantumDependency
            || onTrue.InferredInformation.HasLocalQuantumDependency
            || onFalse.InferredInformation.HasLocalQuantumDependency

        AutoGeneratedExpression kind exType quantumDep

    /// Returns an expression that evaluates whether the two expressions are equal.
    /// The position information is set to null. Does not validate type compatibility.
    let EqualityComparison (lhs: TypedExpression, rhs: TypedExpression) =
        let kind = QsExpressionKind.EQ(lhs, rhs)

        let quantumDep =
            lhs.InferredInformation.HasLocalQuantumDependency
            || rhs.InferredInformation.HasLocalQuantumDependency

        AutoGeneratedExpression kind Bool quantumDep
    ...
    let ArgumentWithControlQubits (arg: TypedExpression) (ctlQs: TypedExpression) =
        ...
        if ctlQs.ResolvedType.Resolution <> QubitArrayType.Resolution && not (ctlQs.ResolvedType |> isInvalid) then
            ArgumentException "expression for the control qubits is valid but not of type Qubit[]" |> raise
    ...
    let ControlledOperation (ex: TypedExpression) =
        let kind = ControlledApplication ex
    ...
    let AdjointOperation (ex: TypedExpression) =
        let kind = AdjointApplication(ex)
