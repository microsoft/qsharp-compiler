...
module SyntaxGenerator =
    ...
    let rec ExpressionAsSymbolTuple (ex : TypedExpression) : SymbolTuple =
        ...
        | ValueTuple items -> items |> Seq.map ExpressionAsSymbolTuple |> ImmutableArray.CreateRange |> VariableNameTuple
        ...
        | _ -> ArgumentException "the given expression contains items that do not represent a valid symbol name" |> raise
