            ...
            try
            {
                ...
                foreach (var declaration in currentlyDefined)
                {
                    ...
                    if (header.Kind.IsTypeConstructor) 
                    {
                        ...
                        var defaultSpec = new QsSpecialization(QsSpecializationKind.QsBody, header.QualifiedName, header.Attributes, QsSpecialization.UnsetRequiredCapability, header.SourceFile, 
                            specLocation, QsNullable<ImmutableArray<ResolvedType>>.Null, header.Signature, SpecializationImplementation.Intrinsic, ImmutableArray<string>.Empty, QsComments.Empty);
                    ...
                    var specializations = this.GlobalSymbols.DefinedSpecializations(header.QualifiedName).Select(defined =>
                    {
                        ...
                        return new QsSpecialization(compiledSpec.Kind, compiledSpec.Parent, compiledSpec.Attributes, QsSpecialization.UnsetRequiredCapability, compiledSpec.SourceFile, 
                            specLocation, compiledSpec.TypeArguments, compiledSpec.Signature, compiledSpec.Implementation, compiledSpec.Documentation, compiledSpec.Comments); 
                    })
        ...
        private QsCallable GetImportedCallable(CallableDeclarationHeader header)
        {
            ...
            var specializations = importedSpecs.Select(imported =>
            {
                ...
                var specSignature = specHeader.Kind.IsQsControlled || specHeader.Kind.IsQsControlledAdjoint 
                    ...
                    : header.Signature;
                return new QsSpecialization(specHeader.Kind, header.QualifiedName, specHeader.Attributes, QsSpecialization.UnsetRequiredCapability, 
                    specHeader.SourceFile, specLocation, specHeader.TypeArguments, specSignature, implementation, specHeader.Documentation, 
                    QsComments.Empty);
            })
            .ToImmutableArray();
            var location = new QsLocation(header.Position, header.SymbolRange);
