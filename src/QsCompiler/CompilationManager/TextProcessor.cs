...
namespace Microsoft.Quantum.QsCompiler.CompilationBuilder
{
    ...
    internal static class TextProcessor
    {
        ...
        private static IEnumerable<Diagnostic> ParseCode(ref List<CodeFragment> fragments, string filename)
        {
            ...
            foreach (var snippet in fragments)
            {
                ...
                for (var outputIndex = 0; outputIndex < outputs.Length; ++outputIndex)
                {
                    ...
                    var fragment = new CodeFragment(snippet.Indentation, fragmentRange, output.Text.Value,
                        outputIndex == outputs.Length - 1 ? snippet.FollowedBy : CodeFragment.MissingDelimiter, output.Kind);
                    ...
                    foreach (var fragmentDiagnostic in output.Diagnostics)
                    {
                        ...
                        if (fragmentDiagnostic.Diagnostic.IsError && diagnosticGoesUpToFragmentEnd) checkEnding = false;
                    }
                    if (checkEnding) diagnostics.AddRange(fragment.CheckFragmentDelimiters(filename));
                }
                if (outputs.Length == 0) processedFragments.Add(snippet); // keep empty fragments around (note that the kind is set to null in this case!)
            }
        ...
        private static string GetCodeSnippet(this FileContentManager file, Range range)
        {
            ...
            if (count == 1) return firstLine.Substring(range.Start.Character, range.End.Character - range.Start.Character);
            ...
            if (middle.Length == 0) return Utils.JoinLines(new string[] { prepend, append });
            else return Utils.JoinLines(new string[] { prepend, Utils.JoinLines(middle), append }); // Note: use JoinLines here to get accurate position infos for errors
        }
        ...
        private static Position FragmentEnd(this FileContentManager file, ref Position current)
        {
            ...
            if (lastInFile.IsSmallerThanOrEqualTo(current)) throw new ArgumentException("no fragment exists at the given position");
            ...
            if (current.Character > text.Length || text.Substring(current.Character).TrimEnd().Length == 0)
            {
                ...
                while (trimmed.Length == 0 && ++current.Line < file.NrLines()) trimmed = file.GetLine(current.Line).WithoutEnding.TrimStart();
                if (current.Line < file.NrLines()) current.Character = file.GetLine(current.Line).WithoutEnding.Length - trimmed.Length;
                else
                {
                    current = lastInFile; 
                    return lastInFile;
        ...
        private static IEnumerable<CodeFragment> FragmentsToProcess(this FileContentManager file, SortedSet<int> changedLines)
        {
            ...
            while (iter.MoveNext())
            {
                ...
                if (processed.Line < iter.Current)
                {
                    ...
                    if (processed.IsSmallerThan(statementStart)) processed = statementStart;
                }
        ...
        internal static Range GetSyntaxCheckDelimiters(this FileContentManager file, int start, int count)
        {
            ...
            if (count < 0 || start + count > file.NrLines()) throw new ArgumentOutOfRangeException(nameof(count));
