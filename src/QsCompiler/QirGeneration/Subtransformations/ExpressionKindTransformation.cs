    ...
    internal class QirExpressionKindTransformation : ExpressionKindTransformation<GenerationContext>
    {
            ...
            bool unreferenceOriginal = false)
        {
            ...
            void StoreElement(PointerValue pointer, IValue value, Value wasCopied, bool shallow = false)
            {
                ...
                {
                    var contBlock = sharedState.FunctionContext.AddBlockAfterCurrent("condContinue");
                    var falseBlock = sharedState.FunctionContext.AddBlockAfterCurrent("condFalse");

                    sharedState.FunctionContext.Emit(b => b.Branch(wasCopied, contBlock, falseBlock));
                    sharedState.ScopeMgr.OpenScope();
                    sharedState.FunctionContext.SetCurrentBlock(falseBlock);
                    ...
                    sharedState.FunctionContext.Emit(b => b.Branch(contBlock));
                    sharedState.FunctionContext.SetCurrentBlock(contBlock);
                }
            ...
            IValue CopyAndUpdateArray(ArrayValue originalArray)
            {
                ...
                var copy = sharedState.FunctionContext.Emit(b => b.Call(createShallowCopy, originalArray.OpaquePointer, forceCopy));
                var array = sharedState.Values.FromArray(copy, originalArray.QSharpElementType);
                var wasCopied = sharedState.FunctionContext.Emit(b => b.Compare(IntPredicate.NotEqual, originalArray.OpaquePointer, array.OpaquePointer));
            ...
            IValue CopyAndUpdateUdt(TupleValue originalValue)
            {
                (Value, TupleValue) GetTupleCopy(TupleValue original)
                {
                    ...
                    var forceCopy = sharedState.Context.CreateConstant(false);
                    var copy = sharedState.FunctionContext.Emit(b => b.Call(createShallowCopy, original.OpaquePointer, forceCopy));
                    ...
                    var wasCopied = sharedState.FunctionContext.Emit(b => b.Compare(IntPredicate.NotEqual, original.OpaquePointer, tuple.OpaquePointer));
        ...
        internal static IValue CreateStringLiteral(GenerationContext sharedState, string str, params IValue[] interpolArgs)
        {
            ...
            Value CreateConstantString(string s)
            {
                ...
                return sharedState.FunctionContext.Emit(b =>
                {
                    ...
                    return b.Call(createString, zeroLengthString);
                });
            }
            ...
            Value DoAppend(Value? curr, Value next, bool unreferenceNext = true)
            {
                ...
                if (curr == null)
                {
                    if (!unreferenceNext)
                    {
                        ...
                        sharedState.FunctionContext.Emit(b => b.Call(refCountUpdate, next, plusOne));
                    }
                    return next;
                }
                ...
                return sharedState.FunctionContext.Emit(b =>
                {
                    var app = b.Call(concatenate, curr, next);
                    b.Call(refCountUpdate, curr, minusOne);
                    if (unreferenceNext)
                    {
                        b.Call(refCountUpdate, next, minusOne);
                    }
                    return app;
                });
            }
            ...
            Value ExpressionToString(IValue evaluated)
            {
                void UpdateStringRefCount(Value str, int change)
                {
                    ...
                    sharedState.FunctionContext.Emit(b => b.Call(addReference, str, countChange));
                }
                ...
                Value SimpleToString(string rtFuncName)
                {
                    ...
                    return sharedState.FunctionContext.Emit(b => b.Call(createString, evaluated.Value));
                }
        ...
        private Value ConditionalEvaluation(Value condition, TypedExpression? onCondTrue = null, TypedExpression? onCondFalse = null)
        {
            ...
            if (onCondTrue != null)
            {
                ...
                this.SharedState.FunctionContext.Emit(b => b.Branch(contBlock));
                (evaluatedOnTrue, afterTrue) = (onTrue.Value, this.SharedState.FunctionContext.CurrentBlock);
            }
            ...
            {
                this.SharedState.ScopeMgr.OpenScope();
                this.SharedState.FunctionContext.SetCurrentBlock(falseBlock);
                ...
                this.SharedState.FunctionContext.Emit(b => b.Branch(contBlock));
                (evaluatedOnFalse, afterFalse) = (onFalse.Value, this.SharedState.FunctionContext.CurrentBlock);
            }

            this.SharedState.FunctionContext.SetCurrentBlock(contBlock);
            var phi = this.SharedState.FunctionContext.Emit(b => b.PhiNode(this.SharedState.CurrentLlvmExpressionType()));
        ...
        private IValue InvokeGlobalCallable(QsQualifiedName callableName, QsSpecializationKind kind, TypedExpression arg)
        {
            IValue CallGlobal(IrFunction func, TypedExpression arg, ResolvedType returnType)
            {
                ...
                var res = this.SharedState.FunctionContext.Emit(b => b.Call(func, argList));
        ...
        private IValue InvokeLocalCallable(TypedExpression method, TypedExpression arg)
        {
            ...
            if (returnType.Resolution.IsUnitType)
            {
                ...
                this.SharedState.FunctionContext.Emit(b => b.Call(func, calledValue.Value, callableArg, resultTuple));
                return this.SharedState.Values.Unit;
            }
            else
            {
                ...
                this.SharedState.FunctionContext.Emit(b => b.Call(func, calledValue.Value, callableArg, resultTuple.OpaquePointer));
        ...
        private CallableValue ApplyFunctor(string runtimeFunctionName, CallableValue callable, bool modifyInPlace = false)
        {
            ...
            if (!modifyInPlace)
            {
                ...
                var modified = this.SharedState.FunctionContext.Emit(b => b.Call(makeCopy, callable.Value, forceCopy));
            ...
            this.SharedState.FunctionContext.Emit(b => b.Call(applyFunctor, callable.Value));
            return callable;
        }
        ...
        private CallableValue CreateCallableValue(ResolvedType callableType, GlobalVariable table, TupleValue? capture = null)
        {
            ...
            var res = this.SharedState.FunctionContext.Emit(b => b.Call(createCallable, table, memoryManagementTable, capture?.OpaquePointer ?? this.SharedState.Constants.UnitValue));
        ...
        public override ResolvedExpressionKind OnAddition(TypedExpression lhsEx, TypedExpression rhsEx)
        {
            ...
            {
                var res = this.SharedState.FunctionContext.Emit(b => b.Add(lhs.Value, rhs.Value));
                value = this.SharedState.Values.FromSimpleValue(res, exType);
            }
            else if (exType.Resolution.IsDouble)
            {
                var res = this.SharedState.FunctionContext.Emit(b => b.FAdd(lhs.Value, rhs.Value));
                value = this.SharedState.Values.FromSimpleValue(res, exType);
            }
            else if (exType.Resolution.IsBigInt)
            {
                ...
                var res = this.SharedState.FunctionContext.Emit(b => b.Call(adder, lhs.Value, rhs.Value));
        ...
        public override ResolvedExpressionKind OnArrayItem(TypedExpression arr, TypedExpression idx)
        {
            ...
            else if (idx.ResolvedType.Resolution.IsRange)
            {
                ...
                var slice = this.SharedState.FunctionContext.Emit(b => b.Call(sliceArray, array.Value, index.Value, forceCopy));
        ...
        public override ResolvedExpressionKind OnBigIntLiteral(BigInteger b)
        {
            ...
            if (b <= long.MaxValue && b >= long.MinValue)
            {
                ...
                var res = this.SharedState.FunctionContext.Emit(b => b.Call(createBigInt, val));
                value = this.SharedState.Values.From(res, exType);
                this.SharedState.ScopeMgr.RegisterValue(value);
            }
            else
            {
                ...
                var byteArrayPointer = this.SharedState.FunctionContext.Emit(b => b.GetElementPtr(
                    this.SharedState.Context.Int8Type.CreateArrayType((uint)bytes.Length),
                    byteArray,
                    new[] { this.SharedState.Context.CreateConstant(0) }));
                var zeroByteArray = this.SharedState.FunctionContext.Emit(b => b.BitCast(
                    byteArrayPointer,
                    this.SharedState.Types.DataArrayPointer));
                var res = this.SharedState.FunctionContext.Emit(b => b.Call(createBigInt, n, zeroByteArray));
                value = this.SharedState.Values.From(res, exType);
                this.SharedState.ScopeMgr.RegisterValue(value);
            }
        ...
        public override ResolvedExpressionKind OnBitwiseAnd(TypedExpression lhsEx, TypedExpression rhsEx)
        {
            ...
            if (exType.Resolution.IsInt)
            {
                var res = this.SharedState.FunctionContext.Emit(b => b.And(lhs.Value, rhs.Value));
                value = this.SharedState.Values.FromSimpleValue(res, exType);
            }
            else if (exType.Resolution.IsBigInt)
            {
                ...
                var res = this.SharedState.FunctionContext.Emit(b => b.Call(func, lhs.Value, rhs.Value));
                value = this.SharedState.Values.From(res, exType);
                this.SharedState.ScopeMgr.RegisterValue(value);
            }
            else
        ...
        public override ResolvedExpressionKind OnBitwiseExclusiveOr(TypedExpression lhsEx, TypedExpression rhsEx)
        {
            ...
            if (exType.Resolution.IsInt)
            {
                var res = this.SharedState.FunctionContext.Emit(b => b.Xor(lhs.Value, rhs.Value));
                value = this.SharedState.Values.FromSimpleValue(res, exType);
            }
            else if (exType.Resolution.IsBigInt)
            {
        ...
        public override ResolvedExpressionKind OnBitwiseNot(TypedExpression ex)
        {
            ...
            if (exType.Resolution.IsInt)
            {
                ...
                var res = this.SharedState.FunctionContext.Emit(b => b.Xor(exValue.Value, minusOne));
                value = this.SharedState.Values.FromSimpleValue(res, exType);
            }
            else if (exType.Resolution.IsBigInt)
            {
                ...
                var res = this.SharedState.FunctionContext.Emit(b => b.Call(func, exValue.Value));
                value = this.SharedState.Values.From(res, exType);
                this.SharedState.ScopeMgr.RegisterValue(value);
            }
            else
        ...
        public override ResolvedExpressionKind OnBitwiseOr(TypedExpression lhsEx, TypedExpression rhsEx)
        {
            ...
            if (exType.Resolution.IsInt)
            {
                var res = this.SharedState.FunctionContext.Emit(b => b.Or(lhs.Value, rhs.Value));
                value = this.SharedState.Values.FromSimpleValue(res, exType);
            }
            else if (exType.Resolution.IsBigInt)
            {
        ...
        public override ResolvedExpressionKind OnConditionalExpression(TypedExpression condEx, TypedExpression ifTrueEx, TypedExpression ifFalseEx)
        {
            ...
            if (ExpressionIsSelfEvaluating(ifTrueEx) && ExpressionIsSelfEvaluating(ifFalseEx))
            {
                ...
                var res = this.SharedState.FunctionContext.Emit(b => b.Select(cond.Value, ifTrue.Value, ifFalse.Value));
                value = this.SharedState.Values.From(res, exType);
            }
            else
            {
        ...
        public override ResolvedExpressionKind OnDivision(TypedExpression lhsEx, TypedExpression rhsEx)
        {
            ...
            if (exType.Resolution.IsInt)
            {
                var res = this.SharedState.FunctionContext.Emit(b => b.SDiv(lhs.Value, rhs.Value));
                value = this.SharedState.Values.FromSimpleValue(res, exType);
            }
            else if (exType.Resolution.IsDouble)
            {
                var res = this.SharedState.FunctionContext.Emit(b => b.FDiv(lhs.Value, rhs.Value));
                value = this.SharedState.Values.FromSimpleValue(res, exType);
            }
            else if (exType.Resolution.IsBigInt)
            {
        ...
        public override ResolvedExpressionKind OnEquality(TypedExpression lhsEx, TypedExpression rhsEx)
        {
            ...
            if (lhsEx.ResolvedType.Resolution.IsResult)
            {
                ...
                var res = this.SharedState.FunctionContext.Emit(b => b.Call(equals, lhs.Value, rhs.Value));
                value = this.SharedState.Values.FromSimpleValue(res, exType);
            }
                ...
                || lhsEx.ResolvedType.Resolution.IsPauli)
            {
                ...
                var res = this.SharedState.FunctionContext.Emit(b => b.Compare(IntPredicate.Equal, lhs.Value, rhs.Value));
                value = this.SharedState.Values.FromSimpleValue(res, exType);
            }
            else if (lhsEx.ResolvedType.Resolution.IsDouble)
            {
                var res = this.SharedState.FunctionContext.Emit(b => b.Compare(RealPredicate.OrderedAndEqual, lhs.Value, rhs.Value));
                value = this.SharedState.Values.FromSimpleValue(res, exType);
            }
            else if (lhsEx.ResolvedType.Resolution.IsString)
            {
                ...
                var res = this.SharedState.FunctionContext.Emit(b => b.Call(compareEquality, lhs.Value, rhs.Value));
                value = this.SharedState.Values.FromSimpleValue(res, exType);
            }
        ...
        public override ResolvedExpressionKind OnExponentiate(TypedExpression lhsEx, TypedExpression rhsEx)
        {
            ...
            if (exType.Resolution.IsInt)
            {
                // TODO: combine some Emit calls
                var baseValue = this.SharedState.FunctionContext.Emit(b => b.SIToFPCast(lhs.Value, this.SharedState.Context.DoubleType));
                var powFunc = this.SharedState.Module.GetIntrinsicDeclaration("llvm.powi.f", this.SharedState.Context.DoubleType);
                var exponent = this.SharedState.FunctionContext.Emit(b => b.IntCast(rhs.Value, this.SharedState.Context.Int32Type, true));
                var resAsDouble = this.SharedState.FunctionContext.Emit(b => b.Call(powFunc, baseValue, exponent));
                var res = this.SharedState.FunctionContext.Emit(b => b.FPToSICast(resAsDouble, this.SharedState.Types.Int));
                value = this.SharedState.Values.FromSimpleValue(res, exType);
            }
            else if (exType.Resolution.IsDouble)
            {
                var powFunc = this.SharedState.Module.GetIntrinsicDeclaration("llvm.pow.f", this.SharedState.Types.Double);
                var res = this.SharedState.FunctionContext.Emit(b => b.Call(powFunc, lhs.Value, rhs.Value));
                value = this.SharedState.Values.FromSimpleValue(res, exType);
            }
            else if (exType.Resolution.IsBigInt)
            {
                ...
                var exponent = this.SharedState.FunctionContext.Emit(b => b.IntCast(rhs.Value, this.SharedState.Context.Int32Type, true));
                var res = this.SharedState.FunctionContext.Emit(b => b.Call(powFunc, lhs.Value, exponent));
                value = this.SharedState.Values.From(res, exType);
                this.SharedState.ScopeMgr.RegisterValue(value);
            }
            else
        ...
        public override ResolvedExpressionKind OnGreaterThan(TypedExpression lhsEx, TypedExpression rhsEx)
        {
            ...
            {
                var res = this.SharedState.FunctionContext.Emit(b => b.Compare(IntPredicate.SignedGreaterThan, lhs.Value, rhs.Value));
                value = this.SharedState.Values.FromSimpleValue(res, exType);
            }
            else if (lhsEx.ResolvedType.Resolution.IsDouble)
            {
                var res = this.SharedState.FunctionContext.Emit(b => b.Compare(RealPredicate.OrderedAndGreaterThan, lhs.Value, rhs.Value));
                value = this.SharedState.Values.FromSimpleValue(res, exType);
            }
            else if (lhsEx.ResolvedType.Resolution.IsBigInt)
            {
        ...
        public override ResolvedExpressionKind OnGreaterThanOrEqual(TypedExpression lhsEx, TypedExpression rhsEx)
        {
            ...
            if (lhsEx.ResolvedType.Resolution.IsInt)
            {
                var res = this.SharedState.FunctionContext.Emit(b => b.Compare(IntPredicate.SignedGreaterThanOrEqual, lhs.Value, rhs.Value));
                value = this.SharedState.Values.FromSimpleValue(res, exType);
            }
            else if (lhsEx.ResolvedType.Resolution.IsDouble)
            {
                var res = this.SharedState.FunctionContext.Emit(b => b.Compare(RealPredicate.OrderedAndGreaterThanOrEqual, lhs.Value, rhs.Value));
                value = this.SharedState.Values.FromSimpleValue(res, exType);
            }
            else if (lhsEx.ResolvedType.Resolution.IsBigInt)
            {
        ...
        public override ResolvedExpressionKind OnInequality(TypedExpression lhsEx, TypedExpression rhsEx)
        {
            ...
            if (lhsEx.ResolvedType.Resolution.IsResult)
            {
                ...
                var res = this.SharedState.FunctionContext.Emit(b => b.Not(b.Call(compareEquality, lhs.Value, rhs.Value)));
                value = this.SharedState.Values.FromSimpleValue(res, exType);
            }
            else if (lhsEx.ResolvedType.Resolution.IsBool || lhsEx.ResolvedType.Resolution.IsInt || lhsEx.ResolvedType.Resolution.IsQubit
                || lhsEx.ResolvedType.Resolution.IsPauli)
            {
                // Works for pointers as well as integer types
                var res = this.SharedState.FunctionContext.Emit(b => b.Compare(IntPredicate.NotEqual, lhs.Value, rhs.Value));
                value = this.SharedState.Values.FromSimpleValue(res, exType);
            }
            else if (lhsEx.ResolvedType.Resolution.IsDouble)
            {
                var res = this.SharedState.FunctionContext.Emit(b => b.Compare(RealPredicate.OrderedAndNotEqual, lhs.Value, rhs.Value));
                value = this.SharedState.Values.FromSimpleValue(res, exType);
            }
            else if (lhsEx.ResolvedType.Resolution.IsString)
            {
        ...
        public override ResolvedExpressionKind OnLeftShift(TypedExpression lhsEx, TypedExpression rhsEx)
        {
            ...
            if (exType.Resolution.IsInt)
            {
                var res = this.SharedState.FunctionContext.Emit(b => b.ShiftLeft(lhs.Value, rhs.Value));
                value = this.SharedState.Values.FromSimpleValue(res, exType);
            }
            else if (exType.Resolution.IsBigInt)
            {
        ...
        public override ResolvedExpressionKind OnLessThan(TypedExpression lhsEx, TypedExpression rhsEx)
        {
            ...
            if (lhsEx.ResolvedType.Resolution.IsInt)
            {
                var res = this.SharedState.FunctionContext.Emit(b => b.Compare(IntPredicate.SignedLessThan, lhs.Value, rhs.Value));
                value = this.SharedState.Values.FromSimpleValue(res, exType);
            }
            else if (lhsEx.ResolvedType.Resolution.IsDouble)
            {
                var res = this.SharedState.FunctionContext.Emit(b => b.Compare(RealPredicate.OrderedAndLessThan, lhs.Value, rhs.Value));
                value = this.SharedState.Values.FromSimpleValue(res, exType);
            }
            else if (lhsEx.ResolvedType.Resolution.IsBigInt)
            {
                var func = this.SharedState.GetOrCreateRuntimeFunction(RuntimeLibrary.BigIntGreaterEq);
                var res = this.SharedState.FunctionContext.Emit(b => b.Not(b.Call(func, lhs.Value, rhs.Value)));
                value = this.SharedState.Values.FromSimpleValue(res, exType);
            }
            else
            {
        ...
        public override ResolvedExpressionKind OnLessThanOrEqual(TypedExpression lhsEx, TypedExpression rhsEx)
        {
            ...
            if (lhsEx.ResolvedType.Resolution.IsInt)
            {
                var res = this.SharedState.FunctionContext.Emit(b => b.Compare(IntPredicate.SignedLessThanOrEqual, lhs.Value, rhs.Value));
                value = this.SharedState.Values.FromSimpleValue(res, exType);
            }
            else if (lhsEx.ResolvedType.Resolution.IsDouble)
            {
                var res = this.SharedState.FunctionContext.Emit(b => b.Compare(RealPredicate.OrderedAndLessThanOrEqual, lhs.Value, rhs.Value));
                value = this.SharedState.Values.FromSimpleValue(res, exType);
            }
            else if (lhsEx.ResolvedType.Resolution.IsBigInt)
            {
        ...
        public override ResolvedExpressionKind OnLogicalAnd(TypedExpression lhsEx, TypedExpression rhsEx)
        {
            ...
            if (ExpressionIsSelfEvaluating(rhsEx))
            {
                var lhs = this.SharedState.EvaluateSubexpression(lhsEx);
                var rhs = this.SharedState.EvaluateSubexpression(rhsEx);
                evaluated = this.SharedState.FunctionContext.Emit(b => b.And(lhs.Value, rhs.Value));
            }
            else
            {
        ...
        public override ResolvedExpressionKind OnLogicalNot(TypedExpression ex)
        {
            ...
            var res = this.SharedState.FunctionContext.Emit(b => b.Not(arg.Value));
            var exType = this.SharedState.CurrentExpressionType();
            var value = this.SharedState.Values.FromSimpleValue(res, exType);
            this.SharedState.ValueStack.Push(value);
            return ResolvedExpressionKind.InvalidExpr;
        ...
        public override ResolvedExpressionKind OnLogicalOr(TypedExpression lhsEx, TypedExpression rhsEx)
        {
            ...
            if (ExpressionIsSelfEvaluating(rhsEx))
            {
                var lhs = this.SharedState.EvaluateSubexpression(lhsEx);
                var rhs = this.SharedState.EvaluateSubexpression(rhsEx);
                evaluated = this.SharedState.FunctionContext.Emit(b => b.Or(lhs.Value, rhs.Value));
            }
            else
            {
                var cond = this.SharedState.EvaluateSubexpression(lhsEx).Value;
        ...
        public override ResolvedExpressionKind OnModulo(TypedExpression lhsEx, TypedExpression rhsEx)
        {
            ...
            if (exType.Resolution.IsInt)
            {
                var res = this.SharedState.FunctionContext.Emit(b => b.SRem(lhs.Value, rhs.Value));
                value = this.SharedState.Values.FromSimpleValue(res, exType);
            }
            else if (exType.Resolution.IsBigInt)
            {
        ...
        public override ResolvedExpressionKind OnMultiplication(TypedExpression lhsEx, TypedExpression rhsEx)
        {
            ...
            if (exType.Resolution.IsInt)
            {
                var res = this.SharedState.FunctionContext.Emit(b => b.Mul(lhs.Value, rhs.Value));
                value = this.SharedState.Values.FromSimpleValue(res, exType);
            }
            else if (exType.Resolution.IsDouble)
            {
                var res = this.SharedState.FunctionContext.Emit(b => b.FMul(lhs.Value, rhs.Value));
                value = this.SharedState.Values.FromSimpleValue(res, exType);
            }
            else if (exType.Resolution.IsBigInt)
            {
        ...
        public override ResolvedExpressionKind OnNegative(TypedExpression ex)
        {
            ...
            if (exType.Resolution.IsInt)
            {
                var res = this.SharedState.FunctionContext.Emit(b => b.Neg(exValue.Value));
                value = this.SharedState.Values.FromSimpleValue(res, exType);
            }
            else if (exType.Resolution.IsDouble)
            {
                var res = this.SharedState.FunctionContext.Emit(b => b.FNeg(exValue.Value));
                value = this.SharedState.Values.FromSimpleValue(res, exType);
            }
            else if (exType.Resolution.IsBigInt)
            {
            ...
            return this.SharedState.FunctionContext.Emit(b =>
            {
                IValue DefaultValue(ResolvedType type)
                {
                    ...
                    else if (type.Resolution.IsPauli)
                    {
                        ...
                        var constant = b.Load(this.SharedState.Types.Pauli, pointer);
                    ...
                    else if (type.Resolution.IsRange)
                    {
                        ...
                        var constant = b.Load(this.SharedState.Types.Range, pointer);
                        return this.SharedState.Values.From(constant, type);
                    }
                    ...
                    else if (type.Resolution.IsString)
                    {
                        ...
                        var value = b.Call(create, this.SharedState.Types.DataArrayPointer.GetNullValue());
        ...
        public override ResolvedExpressionKind OnPartialApplication(TypedExpression method, TypedExpression arg)
        {
            ...
            IrFunction BuildLiftedSpecialization(string name, QsSpecializationKind kind, ImmutableArray<ResolvedType> captureType, ImmutableArray<ResolvedType> paArgsTypes, PartialApplicationArgument partialArgs)
            {
                ...
                void BuildPartialApplicationBody(IReadOnlyList<Argument> parameters)
                {
                    ...
                    this.SharedState.FunctionContext.Emit(b => b.Call(invokeCallable, ApplyFunctors(innerCallable).Value, innerArg.OpaquePointer, parameters[2]));
                }
        ...
        public override ResolvedExpressionKind OnPauliLiteral(QsPauli p)
        {
            ...
            {
                var constant = this.SharedState.FunctionContext.Emit(b => b.Load(this.SharedState.Types.Pauli, pauli));
                var exType = this.SharedState.CurrentExpressionType();
        ...
        public override ResolvedExpressionKind OnResultLiteral(QsResult r)
        {
            ...
            var constant = this.SharedState.FunctionContext.Emit(b => b.Call(getResultLiteral));
            var exType = this.SharedState.CurrentExpressionType();
        ...
        public override ResolvedExpressionKind OnRightShift(TypedExpression lhsEx, TypedExpression rhsEx)
        {
            ...
            if (exType.Resolution.IsInt)
            {
                var res = this.SharedState.FunctionContext.Emit(b => b.ArithmeticShiftRight(lhs.Value, rhs.Value));
                value = this.SharedState.Values.FromSimpleValue(res, exType);
            }
            else if (exType.Resolution.IsBigInt)
            {
        ...
        public override ResolvedExpressionKind OnSubtraction(TypedExpression lhsEx, TypedExpression rhsEx)
        {
            ...
            if (exType.Resolution.IsInt)
            {
                var res = this.SharedState.FunctionContext.Emit(b => b.Sub(lhs.Value, rhs.Value));
                value = this.SharedState.Values.FromSimpleValue(res, exType);
            }
            else if (exType.Resolution.IsDouble)
            {
                var res = this.SharedState.FunctionContext.Emit(b => b.FSub(lhs.Value, rhs.Value));
                value = this.SharedState.Values.FromSimpleValue(res, exType);
            }
            else if (exType.Resolution.IsBigInt)
            {
