    ...
    internal class QirExpressionKindTransformation : ExpressionKindTransformation<GenerationContext>
    {
        ...
        private void BuildPartialApplication(TypedExpression method, TypedExpression arg)
        {
            RebuildItem BuildPartialArgList(ResolvedType argType, TypedExpression arg, List<ResolvedType> remainingArgs, List<(Value, ResolvedType)> capturedValues)
            {
                    ...
                    && argType.Resolution is QsResolvedTypeKind.TupleType types)
                {
                    var itemType = this.SharedState.Context.CreateStructType(
                        false,
                        types.Item.Select(i => this.SharedState.LlvmTypeFromQsharpType(i)).Prepend(this.SharedState.Types.QirTupleHeader).ToArray());
            ...
            IrFunction BuildLiftedSpecialization(string name, QsSpecializationKind kind, ITypeRef captureType, ITypeRef parArgsType, RebuildItem rebuild)
            {
                ...
                var func = this.SharedState.Module.CreateFunction(funcName, this.SharedState.Types.QirCallableSignature);
                ...
                if ((kind == QsSpecializationKind.QsControlled) || (kind == QsSpecializationKind.QsControlledAdjoint))
                {
                    ...
                    var ctlArgsType = this.SharedState.Context.CreateStructType(
                        false,
                        this.SharedState.Types.QirTupleHeader,
                        this.SharedState.Types.QirArray,
                        this.SharedState.Types.QirTuple);
                    ...
                    var destControlsPointer = builder.GetStructElementPointer(ctlArgsType.CreatePointerType(), typedNewTuple, 1u);
                    var controls = builder.Load(this.SharedState.Types.QirArray, controlsPointer);
                    builder.Store(controls, destControlsPointer);
                    var destArgsPointer = builder.GetStructElementPointer(ctlArgsType.CreatePointerType(), typedNewTuple, 2u);
                ...
                var innerCallable = builder.Load(this.SharedState.Types.QirCallable, innerCallablePtr);
            ...
            var parTupleType = paArgTuple.Resolution switch
            {
                QsResolvedTypeKind.TupleType pat => this.SharedState.Context.CreateStructType(
                    false,
                    pat.Item.Select(this.SharedState.LlvmTypeFromQsharpType).Prepend(this.SharedState.Types.QirTupleHeader).ToArray()),
            ...
            var capTypeList = (new ITypeRef[] { this.SharedState.Types.QirTupleHeader, this.SharedState.Types.QirCallable })
            ...
            for (var index = 0; index < 4; index++)
            {
                ...
                {
                    specializations[index] = BuildLiftedSpecialization(liftedName, kind, capType, parTupleType, rebuild);
                }
                else
                {
        ...
        public override ResolvedExpression OnCallLikeExpression(TypedExpression method, TypedExpression arg)
        {
            ...
            void InlineCalledRoutine(QsCallable inlinedCallable, bool isAdjoint, bool isControlled)
            {
                ...
                {
                    this.SharedState.ValueStack.Push(this.SharedState.Types.QirTuple.GetNullValue());
                }
            }
            ...
            bool SupportedByRuntime(QsQualifiedName name)
            {
                ...
                else if (name.Equals(BuiltIn.RangeReverse.FullName))
                {
                    ...
                    var newRange = this.SharedState.CurrentBuilder.Load(
                        this.SharedState.Types.QirRange,
                        this.SharedState.Constants.QirEmptyRange);
            ...
            Value[] BuildControlledArgList(int controlledCount)
            {
                ...
                while (--controlledCount > 0)
                {
                    ...
                    controlArray = this.SharedState.CurrentBuilder.Call(
                        this.SharedState.GetRuntimeFunction("array_concatenate"),
                        controlArray,
            ...
            void CallCallableValue()
            {
                ...
                {
                    argTuple = this.SharedState.Types.QirTuple.GetNullValue();
                }
                else
                {
                    ...
                    argTuple = this.SharedState.CurrentBuilder.BitCast(argStruct, this.SharedState.Types.QirTuple);
                }
                ...
                {
                    resultTuple = this.SharedState.Types.QirTuple.GetNullValue();
        ...
        public override ResolvedExpression OnCopyAndUpdateExpression(TypedExpression lhs, TypedExpression accEx, TypedExpression rhs)
        {
            ...
            else if (lhs.ResolvedType.Resolution is QsResolvedTypeKind.UserDefinedType tt)
            {
                ...
                {
                    this.SharedState.ValueStack.Push(Constant.UndefinedValueFor(this.SharedState.Types.QirTuple));
                }
            }
            else
            {
                this.SharedState.ValueStack.Push(Constant.UndefinedValueFor(this.SharedState.Types.QirInt));
            }
            return ResolvedExpression.InvalidExpr;
        }
        ...
        public override ResolvedExpression OnEquality(TypedExpression lhs, TypedExpression rhs)
        {
            ...
            if (lhs.ResolvedType.Resolution.IsResult)
            {
                ...
                var value = this.SharedState.CurrentBuilder.Call(this.SharedState.GetRuntimeFunction("result_equal"), lhsValue, rhsValue);
                this.SharedState.ValueStack.Push(value);
            }
            ...
            else if (lhs.ResolvedType.Resolution.IsString)
            {
                ...
                var value = this.SharedState.CurrentBuilder.Call(this.SharedState.GetRuntimeFunction("string_equal"), lhsValue, rhsValue);
                this.SharedState.ValueStack.Push(value);
            }
            else if (lhs.ResolvedType.Resolution.IsBigInt)
            {
                // Generate a call to the bigint equality testing function
                var value = this.SharedState.CurrentBuilder.Call(this.SharedState.GetRuntimeFunction("bigint_equal"), lhsValue, rhsValue);
                this.SharedState.ValueStack.Push(value);
            }
            else
            {
        ...
        public override ResolvedExpression OnExponentiate(TypedExpression lhs, TypedExpression rhs)
        {
            ...
            {
                var powFunc = this.SharedState.Module.GetIntrinsicDeclaration("llvm.pow.f", this.SharedState.Types.QirDouble);
                this.SharedState.ValueStack.Push(this.SharedState.CurrentBuilder.Call(powFunc, lhsValue, rhsValue));
            }
            else if (lhs.ResolvedType.Resolution.IsBigInt)
            {
        ...
        public override ResolvedExpression OnIdentifier(Identifier sym, QsNullable<ImmutableArray<ResolvedType>> tArgs)
        {
            ...
            else if (sym is Identifier.GlobalCallable globalCallable)
            {
                if (this.SharedState.TryFindGlobalCallable(globalCallable.Item.Namespace, globalCallable.Item.Name, out QsCallable? callable))
                {
                    ...
                    var func = this.SharedState.GetRuntimeFunction("callable_create");
                    var callableValue = this.SharedState.CurrentBuilder.Call(func, wrapper, this.SharedState.Types.QirTuple.GetNullValue());
                ...
                {
                    this.SharedState.ValueStack.Push(Constant.UndefinedValueFor(this.SharedState.Types.QirCallable));
                }
            }
            else
            {
        ...
        public override ResolvedExpression OnInequality(TypedExpression lhs, TypedExpression rhs)
        {
            ...
            {
                // Generate a call to the result equality testing function
                var eq = this.SharedState.CurrentBuilder.Call(this.SharedState.GetRuntimeFunction("result_equal"), lhsValue, rhsValue);
            ...
            {
                // Generate a call to the string equality testing function
                var eq = this.SharedState.CurrentBuilder.Call(this.SharedState.GetRuntimeFunction("string_equal"), lhsValue, rhsValue);
                var ineq = this.SharedState.CurrentBuilder.Not(eq);
                this.SharedState.ValueStack.Push(ineq);
            }
            else if (lhs.ResolvedType.Resolution.IsBigInt)
            {
                // Generate a call to the bigint equality testing function
                var eq = this.SharedState.CurrentBuilder.Call(this.SharedState.GetRuntimeFunction("bigint_equal"), lhsValue, rhsValue);
                var ineq = this.SharedState.CurrentBuilder.Not(eq);
                this.SharedState.ValueStack.Push(ineq);
            }
            else
...
#pragma warning restore CS0618 // Type or member is obsolete
        ...
        public override ResolvedExpression OnPauliLiteral(QsPauli p)
        {
            void LoadAndPushPauli(Value pauli) =>
                this.SharedState.ValueStack.Push(this.SharedState.CurrentBuilder.Load(this.SharedState.Types.QirPauli, pauli));

            if (p.IsPauliI)
            {
                LoadAndPushPauli(this.SharedState.Constants.QirPauliI);
            }
            else if (p.IsPauliX)
            {
                LoadAndPushPauli(this.SharedState.Constants.QirPauliX);
            }
            else if (p.IsPauliY)
            {
                LoadAndPushPauli(this.SharedState.Constants.QirPauliY);
            }
            else if (p.IsPauliZ)
            {
                LoadAndPushPauli(this.SharedState.Constants.QirPauliZ);
            }
            else
            {
                this.SharedState.ValueStack.Push(Constant.UndefinedValueFor(this.SharedState.Types.QirPauli));
            }
        ...
        public override ResolvedExpression OnRangeLiteral(TypedExpression lhs, TypedExpression rhs)
        {
            ...
            Value rangePtr = this.SharedState.Constants.QirEmptyRange;
            Value range = this.SharedState.CurrentBuilder.Load(this.SharedState.Types.QirRange, rangePtr);
        ...
        {
            var valuePtr = r.IsOne ? this.SharedState.Constants.QirResultOne : this.SharedState.Constants.QirResultZero;
            var value = this.SharedState.CurrentBuilder.Load(this.SharedState.Types.QirResult, valuePtr);
            this.SharedState.ValueStack.Push(value);
        ...
        public override ResolvedExpression OnValueArray(ImmutableArray<TypedExpression> vs)
        {
            ...
            {
                var pointer = this.SharedState.CurrentBuilder.Call(
                    this.SharedState.GetRuntimeFunction("array_get_element_ptr_1d"),
                    array,
        ...
        public override ResolvedExpression OnValueTuple(ImmutableArray<TypedExpression> vs)
        {
            ...
            for (int i = 0; i < vs.Length; i++)
            {
                ...
                var itemPointer = this.SharedState.CurrentBuilder.GetStructElementPointer(tupleType, tuplePointer, (uint)i + 1);
