...
let AllPathsReturnValueOrFail body = 
    ...
    let rec checkReturnStatements withinQubitScope (scope : QsScope) = 
        ...
        for statement in scope.Statements do 
            ...
            | QsStatementKind.QsConditionalStatement statement -> 
                ...
                for (_, case) in statement.ConditionalBlocks do case.Body |> delayAddingReturns |> added.AddRange
    ...
    let rec checkTermination (scope : QsScope) = 
        let isNonTerminatingStatement (qsStatement : QsStatement) = 
            ...
            | QsStatementKind.QsConditionalStatement statement -> 
                let returns = statement.ConditionalBlocks |> Seq.map (fun (_,case) -> checkTermination case.Body) |> Seq.toList
                match statement.Default with
        ...
        if returnOrFailAndAfter.Length <> 0 then 
            let unreachable = returnOrFailAndAfter.[0].Statement |> function
                | QsStatementKind.QsRepeatStatement statement -> statement.FixupBlock.Body.Statements.Concat(returnOrFailAndAfter.Skip(1))
