...
/// Returns true if the given expression contains an identifier 
/// that is not part of a call-like expression and refers to the given parent. 
/// Returns false otherwise.
let internal IsTypeParamRecursion parent ex = 
    let paramSelf = function
        | Identifier (GlobalCallable id, Null) -> id = parent
        | Identifier (GlobalCallable id, Value tArgs) -> id = parent && tArgs.Contains (MissingType |> ResolvedType.New)
        | _ -> false
    let typedEx = AutoGeneratedExpression ex InvalidType false // the expression type is irrelevant
...
let internal VerifyAssignment expectedType parent mismatchErr addError (rhsType, rhsEx, rhsRange) =
    // we need to check if the right hand side contains a type parametrized version of the parent callable
    let directRecursion = IsTypeParamRecursion parent rhsEx 
    ...
    member this.Resolve ({ Symbols = symbols } as context) addDiagnostic : TypedExpression =
        ...
        let buildCopyAndUpdate (lhs : QsExpression, accEx : QsExpression, rhs : QsExpression) =
            ...
            | UserDefinedType _, Identifier (sym, Null) -> 
                ...
                VerifyAssignment itemType symbols.Parent ErrorCode.TypeMismatchInCopyAndUpdateExpr addError (resRhs.ResolvedType, resRhs.Expression, rhs.RangeOrDefault)           
            ...
            | _ -> // by default, assume that the update expression is supposed to be for an array
                ...
                | None -> // indicates a trivial slicing of the form "..." resulting in a complete replacement
                    ...
                    VerifyAssignment expectedRhs symbols.Parent ErrorCode.TypeMismatchInCopyAndUpdateExpr addError (resRhs.ResolvedType, resRhs.Expression, rhs.RangeOrDefault) 
