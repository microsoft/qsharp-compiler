...
and Namespace private
    ...
    member internal this.TryGetAttributeInSource source (attName, possibleQualifications : _ seq) = 
        ...
        let coreAttributeName = NonNullable<string>.New "Attribute"
        let marksAttribute (t : UserDefinedType) = t.Namespace.Value = coreNamespace.Value && t.Name.Value = coreAttributeName.Value
        ...
        let compareAttributeName (sym : QsSymbol) = sym.Symbol |> function 
            | Symbol sym when sym.Value = coreAttributeName.Value && possibleQualifications.Contains "" -> true
            | QualifiedSymbol (ns, sym) when sym.Value = coreAttributeName.Value && possibleQualifications.Contains ns.Value -> true
            | _ -> false
