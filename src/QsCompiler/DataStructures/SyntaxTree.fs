...
| Generated of QsGeneratorDirective // Invert and Distribute will be replaced by Provided before sending to code gen
    with
    static member CreateIntrinsic() = Intrinsic
    static member CreateExternal() = External
    member this.TryGetProvided(provided: (QsTuple<LocalVariableDeclaration<QsLocalSymbol>> * QsScope) byref) =
        match this with
        | Provided(tuple, implementation) -> provided <- (tuple, implementation); true
        | _ -> false
    member this.TryGetGenerated(generated: QsGeneratorDirective byref) =
        match this with
        | Generated value -> generated <- value; true
        | _ -> false
