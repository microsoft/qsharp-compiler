...
type ExternalRewriteStepsManagerTests () =
    ...
    member this.``Steps are ordered`` () =
        ...
        let config = new CompilationLoader.Configuration(RewriteStepTypes = [(typedefof<TestRewriteStep>, "")], RewriteStepInstances = [(stepInstance1 :> IRewriteStep, "");(stepInstance2 :> IRewriteStep, "")])
        let loadedSteps = GetSteps config
