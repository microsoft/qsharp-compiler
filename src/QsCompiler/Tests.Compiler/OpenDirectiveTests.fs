...
let ``Duplicate open directives with conflicting aliases`` () =
    let compilation =
        TestUtils.buildFiles folder [ "DuplicateAndConflictingAliases.qs" ] [] None false TestUtils.Library

    Diagnostics.assertMatches [ Error ErrorCode.InvalidNamespaceAliasName, None ] (compilation.Diagnostics())

[<Fact>]
let ``Open with and without alias`` () =
    let compilation = TestUtils.buildFiles folder [ "OpenWithAlias.qs" ] [] None false TestUtils.Library
    Assert.Empty(compilation.Diagnostics())

[<Fact>]
let ``Duplicated open directives`` () =
    let compilation = TestUtils.buildFiles folder [ "DuplicateOpens.qs" ] [] None false TestUtils.Library
    Assert.Empty(compilation.Diagnostics())
