...
let private createArgument (name: string, argType: DataType, position: int) =
    new Parameter(Name = name, Type = argType, Position = position)
...
let private createEntryPointOperation (name: string, argsList) =
    ...
    List.iter (fun a -> entryPointOperation.Parameters.Add a) argsList
    entryPointOperation

let private createExecutionInformation
    (
    ...
    Map
        ...
        .Add("UseNoArgs",
             createExecutionInformation (
                 createEntryPointOperation ("UseNoArgs", []),
                 new Dictionary<string, ArgumentValue>()
             ))
        .Add(
            "UseBoolArgWithValues",
            createExecutionInformation (
                createEntryPointOperation ("UseBoolArgWithValues", [ createArgument ("BoolArg", DataType.BoolType, 0) ]),
        ...
        .Add(
            ...
            createExecutionInformation (
                ...
                createArgumentMap (
                    "PauliArg",
                    new ArgumentValue(Pauli = new System.Nullable<PauliValue>(PauliValue.PauliY))
                )
            )
        )
        .Add(
        ...
        .Add(
            ...
            createExecutionInformation (
                ...
                createArgumentMap (
                    "ResultArg",
                    new ArgumentValue(Result = new System.Nullable<ResultValue>(ResultValue.One))
                )
            )
        )
        .Add(
